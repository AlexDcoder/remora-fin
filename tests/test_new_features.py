...
def test_cost_service_uses_cache(mock_aws_session_class):
    ...
    mock_df = pl.DataFrame({"unblended_cost": [10.0], "date": [date(2023, 1, 1)], "service": ["S3"], "account": ["123"]})
...
def test_governance_service_instantiation(mock_aws_session_class):
    mock_aws_session = MagicMock()
    mock_aws_session_class.get_instance.return_value = mock_aws_session
    
    from remora.services.governance_service import GovernanceService
    service = GovernanceService(session=mock_aws_session)
