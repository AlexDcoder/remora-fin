...
class SQSService(BaseService):
    ...
    def __init__(self, session: AWSSession | None = None, cache: CacheService | None = None) -> None:
        ...
        super().__init__("sqs", session, cache)
    ...
    async def list_queues_async(self, use_cache: bool = True) -> list[dict[str, Any]]:
        """Async version of list_queues."""
        query = {"service": "sqs", "action": "list_queues", "region": self._session.region}

        async def _fetch():
            queues = []
            async with self._session.async_client("sqs") as sqs:
                paginator = sqs.get_paginator("list_queues")
            ...
            logger.info("Found [bold cyan]%d[/] SQS queues (async)", len(queues))
            return queues
