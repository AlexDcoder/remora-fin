...
class EMRService(BaseService):
    ...
    def __init__(self, session: AWSSession | None = None, cache: CacheService | None = None) -> None:
        ...
        super().__init__("emr", session, cache)
    ...
    async def list_clusters_async(self, use_cache: bool = True) -> list[dict[str, Any]]:
        ...
        async def _fetch():
            clusters = []
            async with self._session.async_client("emr") as emr:
                paginator = emr.get_paginator("list_clusters")
            ...
            logger.info("Found [bold cyan]%d[/] EMR clusters (async)", len(clusters))
            return clusters
