...
class GovernanceService(BaseService):
    ...
    def __init__(self, session: AWSSession | None = None, cache: CacheService | None = None) -> None:
        super().__init__("governance", session, cache)
    ...
    def get_tag_compliance(self, required_tags: list[str], use_cache: bool = True) -> dict[str, Any]:
        ...
        if use_cache:
            cached = self._cache.get_json(self.cache_query(query), max_age_hours=2)
        ...
        if use_cache:
            self._cache.set_json(self.cache_query(query), result)
